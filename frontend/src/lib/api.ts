...
export interface AdminCustomerRecoveryPreview { userId: string; name: string; email: string; whatsapp: string | null; campaignCode: 'REGISTERED_NO_EVENT' | 'EVENT_CREATED_NO_PLAN' | 'CHECKOUT_ABANDONED'; eligibleAt: string; }
export interface AdminCustomerFollowUp { id: string; userId: string; name: string; email: string | null; whatsapp: string | null; campaignCode: AdminCustomerRecoveryPreview['campaignCode']; status: 'PROCESSING' | 'FAILED'; attemptCount: number; scheduledAt: string; nextAttemptAt: string | null; sentAt: string | null; lastError: string | null; }
