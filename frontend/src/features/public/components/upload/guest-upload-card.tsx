  ...
  return (
    <section id="upload" className="grid gap-6 lg:grid-cols-[0.86fr_1.14fr]">
      ...
      {uploadAvailable ? (
        ...
        <div className="flex min-h-[420px] flex-col items-center justify-center rounded-[24px] border border-[var(--event-border-color)] bg-white p-8 text-center shadow-[0_22px_60px_var(--event-primary-mist-color)]">
          <div className="grid size-16 place-items-center rounded-[20px] bg-[var(--event-accent-soft-color)] text-2xl text-[var(--event-accent-ink-color)]">♡</div>
          <h2 className="mt-6 font-display text-[40px] font-semibold leading-none tracking-[-0.045em] text-[var(--event-foreground-color)]">
            Os envios estarão disponíveis em breve
          </h2>
          <p className="mt-4 max-w-md text-sm leading-7 text-[var(--event-muted-foreground-color)]">
