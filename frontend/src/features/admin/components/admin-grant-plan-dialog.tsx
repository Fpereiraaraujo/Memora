...
export function AdminGrantPlanDialog({ customer, busy, onClose, onSubmit }: AdminGrantPlanDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-[70] grid place-items-center bg-[#201914]/35 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="grant-plan-title">
      <form onSubmit={handleSubmit} className="w-full max-w-xl rounded-[2rem] border border-[#efd6c9] bg-[#fffdfb] p-6 shadow-[0_28px_100px_rgba(47,30,22,0.24)] sm:p-8">
        ...
        <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button type="button" onClick={onClose} disabled={busy} className="rounded-2xl border border-[#ead1c4] px-5 py-3 text-sm font-bold text-[#644e43]">Cancelar</button><button type="submit" disabled={busy || !customer.events.length} className="rounded-2xl bg-[#ef7885] px-5 py-3 text-sm font-bold text-white shadow-[0_14px_28px_rgba(239,120,133,0.22)] disabled:cursor-not-allowed disabled:opacity-60">{busy ? 'Liberando plano...' : 'Liberar plano'}</button></div>
      </form>
    </div>
  );
}
