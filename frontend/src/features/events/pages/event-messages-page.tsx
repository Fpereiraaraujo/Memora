...
export function EventMessagesPage() {
  ...
  const totalPages = Math.max(1, Math.ceil(dashboard.messagePhotos.length / MESSAGES_PER_PAGE));

  const visibleMessages = useMemo(() => {
    const start = (currentPage - 1) * MESSAGES_PER_PAGE;
    return dashboard.messagePhotos.slice(start, start + MESSAGES_PER_PAGE);
  }, [currentPage, dashboard.messagePhotos]);
      ...
      {dashboard.event ? (
        <>
          <EventPageHero
            ...
            description="Acompanhe tudo o que seus convidados escreveram junto com os envios de fotos. Esse espaço funciona como um mural carinhoso do evento."
            badge={`${dashboard.messagePhotos.length} recados`}
          />
          ...
          <section className="rounded-[24px] border border-[#f1ddd1] bg-white p-6 shadow-[0_22px_60px_rgba(96,60,36,0.08)]">
            {visibleMessages.length > 0 ? (
              <div className="divide-y divide-[#f0ded4]">
                {visibleMessages.map((photo) => (
                  <article key={photo.id} className="flex gap-4 py-5 first:pt-0 last:pb-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-3 text-sm leading-7 text-[#2c2927]/74">{photo.guestMessage}</p>
                    </div>
                  </article>
                ))}
              </div>
